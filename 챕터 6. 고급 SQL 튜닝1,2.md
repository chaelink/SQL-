...
- Array Processing 가능
- 버퍼캐시 경유가 더 성능을 떨어뜨리기도 함
- 배치 프로그램은 OLTP 시스템에서도 사용함
- 항상 전체 처리속도 최적화에 목표를 두고 튜닝
- 필요 이상의 병렬도(DOP)를 지정하면 안됨
...
- Inter Operation Parallelism
    - 한편의 서버집합에 속한 프로세스들이 읽은 데이터를, 반대편 서버 집합에 전송하는 작업을 병렬로 동시에 실행하는 것
...
- P -> P(PARALLEL_TO_PARALLEL) 오퍼레이션
    ...
    - 데이터를 재분배하는 오퍼레이션
    - 2개의 서버 집합이 처리 -> 사용자가 지정한 병렬도의 2배수만큼 병렬 프로세스 필요
    - 데이터 정렬, 그룹핑, 조인 등에 큐 사용
    - 병렬 프로세스 간 통신 발생 -> Inter Operation Parallelism
...
병렬처리
- 온라인 트랜잭션이 발생하는 테이블에 병렬dml을 사용해서는 안된다
- nl조인도 병렬처리가 가능하다

<br>

병렬 처리(Parallel Query) 시 QC(Query Coordinator)
- 사용자의 SQL 요청을 받아 병렬 서버 프로세스(PQ)에** 작업을 분할/할당**하고, **결과를 취합하여 최종 반환**하는 총괄 프로세스
- 대용량 데이터 처리 속도를 향상시키기 위해 **병렬 서버들 간의 통신과 작업을 감독하는 핵심 역할을 수행**
- 병렬 서버 집합 할당
- 각 병렬 프로세스에 작업 할당
- 병렬도를 지정하지 않은 테이블 직접 읽기
- 병렬 처리 자원을 os에 반환(X) -> 병렬 처리에 사용한 프로세스는 모든 처리 종료 후 일괄 해제 및 반환됨
...
- Granule
    ...
    - 블록 기반 Granule
        - 테이블 풀 스캔, 인덱스 패스트 풀 스캔 경우
        - 파티션보다 더 많은 병렬도 지정 가능 
    - 파티션 기반 Granule
        - 인덱스 레인지 스캔을 병렬로 처리하는 경우
        - 파티션 개수보다 많은 병렬도 지정은 서버 리소스 낭비
...
- full partition wise 조인
    - 하나의 서버집합만 필요
    - 파티션 개수보다 작거나 같은 개수의 dop를 지정해야함
    - 테이블 파티션 방식은 모든 종류 가능
    - 일반적으로 해시 조인을 사용하지만 다른 조인도 사용 가능
- partial partition wise 조인

### 6절 고급 SQL 활용

- WITH절
    - 복잡한 쿼리문에서 반복 사용되는 서브쿼리나 **임시 테이블에 이름을 부여**하여, 쿼리의 가독성을 높이고 성능을 최적화하는 기능
    - **Materialize 방식** : 내부적으로 메모리에 임시 테이블을 생성하여 반복 재사용
        - 사용 케이스
            - WITH절 집합을 두 번 이상 참조
            - 실행계획에 TEMP TABLE TRANSFORMATION 오퍼레이션이 있으면
            - 매우 많은 데이터를 읽어 GROUP BY, 조인 등을 통해 집합 크기를 많이 줄일 수 있을때
            - ! 원본 집합이 작고, **동시 수행 빈도가 높을 때 사용하면 성능에 안좋음**
            - ! 수행빈도가 높지 않더라도 WITH절에서 참조하는 **원본 집합과 결과집합의 크기 차이가 없을 때 사용하면 성능에 안좋음**
    - **Inline 방식** : 물리적으로 임시 테이블을 생성하지 않고 참조된 횟수만큼 런타임 시 반복 수행
...
- Inter Operation Parallelism
    - 한편의 서버집합에 속한 프로세스들이 읽은 데이터를, 반대편 서버 집합에 전송하는 작업을 병렬로 동시에 실행하는 것 ? 이해가 안됨
- full partition wise 조인 , partial partition wise 조인 이해하기
- 병렬 처리에서의 대이터 재분배 방식
    - Broadcast 방식
    - Hash 방식 ?
    - 어떤 종류들이 있고 언제 어떤 방식을 써야 하는지 알아볼것
- 배치 프로그램과 병렬 처리는 다른 것? 구분해서 특징 기억하기
- 병렬 처리와 병렬 DML은 어떻게 다른가?
- WITH절 내용 이해하기
